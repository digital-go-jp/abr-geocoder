# ABR Geocoder v2.2 内部仕様（Node.js 中級者向け）

このドキュメントは、ABR Geocoder の内部アーキテクチャ、設計思想、並列処理、そして SQLite を起点に生成されるキャッシュ（ABRG2 バイナリ辞書）の仕様をまとめたものです。公開前提の日本語ドキュメントで、Mermaid 図で構造を可視化しています。
