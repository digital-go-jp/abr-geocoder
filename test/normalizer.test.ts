...
const cases: TestCase[] = JSON.parse(fs.readFileSync(path.join(__dirname, 'normalizer.cases.json'), 'utf-8'));
const fuzzyCases: TestCase[] = JSON.parse(fs.readFileSync(path.join(__dirname, 'normalizer-fuzzy.cases.json'), 'utf-8'));
...
describe(`normalizer.ts`, () => {
  ...
  describe('fuzzy', () => {
    for (const [addressString, expectedResult] of fuzzyCases) {
      test(addressString, async () => {
        const res = await normalizer.normalizeAddress(addressString, { fuzzy: true });
        expect(res).toEqual(
          expect.objectContaining(expectedResult),
        );
      });
