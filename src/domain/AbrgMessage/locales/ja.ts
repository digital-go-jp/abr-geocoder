...
const messages: Record<AbrgMessage, string> = {
  ...
  [AbrgMessage.CLI_GEOCODE_FORMAT_OPTION]: "出力フォーマットを指定します。デフォルトは'csv'",
  [AbrgMessage.APPLICATION_DESC]: 'デジタル庁：アドレス・ベース・レジストリを用いたジオコーダー',
  [AbrgMessage.CLI_UPDATE_CHECK_DESC]: 'データセットのアップデートを確認します',
  [AbrgMessage.ERROR_NO_UPDATE_IS_AVAILABLE]: '現状データが最新です。更新を中断します。',
  ...
  [AbrgMessage.NEW_DATASET_IS_AVAILABLE]: 'ローカルのデータが更新できます。 abrg download で更新してください',
  [AbrgMessage.DATA_DOWNLOAD_ERROR]: 'データの取得に失敗しました',
  [AbrgMessage.CANNOT_FIND_THE_SPECIFIED_RESOURCE]: '指定されたリソースが見つかりませんでした',
  [AbrgMessage.DOWNLOADED_DATA_DOES_NOT_CONTAIN_THE_RESOURCE_CSV]: '指定されたリソースには、CSVファイルが含まれていませんでした',
