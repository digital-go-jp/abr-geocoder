...
export const fsIterator = async (
  ...
  for await (const d of await fs.promises.opendir(targetDir)) {
    ...
    if (d.isDirectory()) {
      (
        await fsIterator(tmpDir, filePath, targetExtention, fileLoadingProgress)
      ).forEach(result => {
        results.push(result);
      });
      continue;
    ...
    for await (const zipEntry of Object.values(entries)) {
      if (zipEntry.name.endsWith('.zip')) {
        const dstDir = `${tmpDir}/${d.name.replace(
          /[^a-z0-9_]/gi,
          ''
        )}_${zipEntry.name.replace(/[^a-z0-9_]/gi, '')}`;
        await fs.promises.mkdir(dstDir);
        const dst = `${dstDir}/${zipEntry.name}`;
        await zip.extract(zipEntry, dst);
        (
          await fsIterator(tmpDir, dstDir, targetExtention, fileLoadingProgress)
        ).forEach(result => {
          results.push(result);
        });
      } else {
