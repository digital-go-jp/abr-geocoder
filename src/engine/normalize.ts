...
async function normalizeTownName(
  inputOther: string,
  pref: string,
  city: string,
  fuzzy: boolean
): Promise<NormalizeTownNameOutput | undefined> {
...
export const normalize: Normalizer = async (
  ...
  let level = 0;
  let normalized:
    | NormalizeTownNameOutput
    | NormalizeResidentialPartOutput
    | undefined;
  ...
  for (const [prefectureCity, reg] of sameNamedPrefectureCityRegexPatterns) {
    const match = other.match(option.fuzzy ? insertWildcardMatching(reg) : reg);
    if (match) {
  ...
  if (option.level > 3 && normalized && town) {
    const rsdtNormalized = await normalizeResidentialPart(
      other,
      pref,
      city,
      town
